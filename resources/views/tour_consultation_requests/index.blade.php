            ...
            <table id="example" class="table-auto w-full mt-6 border-collapse">
                ...
                <tbody>
                    ...
                    <tr class="bg-white hover:bg-gray-50">
                        <form action="{{ route('tour-consultation-requests.update', $request->id) }}" method="POST"
                            class="update-form">
                            @csrf
                            @method('PUT')
                            <td class="px-4 py-2 border">
                                <textarea name="" class="w-full border border-gray-300 rounded px-2 py-1 resize-none"
                                    disabled>{{ $request->id }}</textarea>
                            </td>
                            <td class="px-4 py-2 border">
                                <textarea name="name"
                                    class="w-full border border-gray-300 rounded px-2 py-1 resize-none"
                                    disabled>{{ $request->name }}</textarea>
                            </td>
                            <td class="px-4 py-2 border">
                                <textarea name="number"
                                    class="w-full border border-gray-300 rounded px-2 py-1 resize-none"
                                    disabled>{{ $request->number }}</textarea>
                            </td>
                            <td class="px-4 py-2 border">
                                <textarea name="address"
                                    class="w-full border border-gray-300 rounded px-2 py-1 resize-none"
                                    disabled>{{ $request->address }}</textarea>
                            </td>
                            <td class="px-4 py-2 border">
                                <textarea name="additional_info"
                                    class="w-full border border-gray-300 rounded px-2 py-1 resize-none"
                                    disabled>{{ $request->additional_info }}</textarea>
                            </td>
                            <td class="px-4 py-2 border">
                                <textarea name="property_name"
                                    class="w-full border border-gray-300 rounded px-2 py-1 resize-none"
                                    disabled>{{ $request->property_name }}</textarea>
                            </td>
                        ...
                        <!-- Delete Button -->
                        <form action="{{ route('tour-consultation-requests.destroy', $request->id) }}" method="POST"
                            class="inline-block">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
