<x-app-layout>
    <div class="container w-[95%] mx-auto">
        ...
        <div class="flex item-center justify-between w-[90%] mx-auto mt-3">
                                ...
                                <div class="summaryRow grid grid-cols-2 gap-4 mb-4">
                                    <div class="form-group">
                                        <label for="attribute_name"
                                            class="block text-sm font-medium text-gray-700">Attribute Name<span
                                    ...
                                    <div class="form-group">
                                        <label for="value" class="block text-sm font-medium text-gray-700">Value<span
                                                class="text-red-500 font-bold text-2xl">*</span></label>
            ...
            <table id="example" class="table-auto w-full mt-6 border-collapse">
                ...
                <tbody>
                    ...
                    <tr class="bg-white hover:bg-gray-50">
                        <form action="{{ route('contact-attributes.update', $attribute->contact_id) }}" method="POST"
                            class="update-form">
                            @csrf
                            @method('PUT')

                            <td>
                                <textarea name="id" class="w-full border border-gray-300 rounded px-2 py-1 resize-none"
                                    disabled>{{ $attribute->contact_id }}</textarea>
                            </td>
                            <td>
                                <textarea name="attribute_name"
                                    class="w-full border border-gray-300 rounded px-2 py-1 resize-none"
                                    disabled>{{ $attribute->attribute_name }}</textarea>
                            </td>
                            <td>
                                <textarea name="value"
                                    class="w-full border border-gray-300 rounded px-2 py-1 resize-none"
                                    disabled>{{ $attribute->value }}</textarea>
                            </td>
                        ...
                        <!-- Delete Button -->
                        <form action="{{ route('contact-attributes.destroy', $attribute->contact_id) }}" method="POST"
                            class="inline-block">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
